namespace Menees.Diffs
{
	...
	internal sealed class StringHasher
	{
		...
		[
#pragma warning disable MEN018 // Use digit separators. Clearer using original CRC numeric literals.
...
#pragma warning restore MEN018 // Use digit separators
		];
		...
		public int GetHashCode(string line)
		{
			...
			if (this.leadingCharactersToIgnore > 0)
			{
				if (this.leadingCharactersToIgnore >= line.Length)
				{
					line = string.Empty;
				}
				else
				{
					line = line.Substring(this.leadingCharactersToIgnore);
				}
			}
			...
			switch (this.hashType)
			{
				...
				case HashType.Crc32:
					result = GetCrc32(line);
					break;

				default:
					result = this.GetUnique(line);
					break;
			}
		...
		private static int GetCrc32(string line)
		{
			...
			unchecked
			{
				const uint AllBitsOnInUint = 0x_FFFF_FFFF;
				uint result = AllBitsOnInUint;
				int numBytes = bytes.Length;
				for (int i = 0; i < numBytes; i++)
				{
					const int BitsPerByte = 8;
					const int AllBitsOnInByte = 0xFF;
					result = (result >> BitsPerByte) ^ Crc32Code[bytes[i] ^ (result & AllBitsOnInByte)];
				}
