namespace Menees.Shell
{
	...
	public sealed class CommandLine
	{
		...
		{
			Conditions.RequireReference(args, nameof(args));
		...
		public static string Build(params object[] arguments)
		{
			...
			if (arguments != null && arguments.Length > 0)
			{
				...
				foreach (object arg in arguments)
				{
					...
					else if (arg is Tuple<string, object> tuple)
					{
						name = tuple.Item1;
						value = tuple.Item2;
					}
		...
		public CommandLine AddValueHandler(Action<string, IList<string>> handler, params KeyValuePair<string, string>[] valueHelp)
		{
			...
			Conditions.RequireReference(handler, nameof(handler));
		...
		private sealed class Switch
		{
			...
			public Switch(string[] names, Delegate handler, CommandLineSwitchOptions options)
			{
				...
				Conditions.RequireArgument(names != null && names.Length > 0, "The switch names array must be non-empty.", nameof(names));
				Conditions.RequireReference(handler, nameof(handler));
				...
				Conditions.RequireArgument(
					!this.IsFlagSwitch || !this.AllowMultiple,
					"Flag switches should never use the AllowMultiple option.",
					nameof(options));
			}
